...
def exponential_chan(s, mtx, N=4096): 
    '''
...
def weight_chan(cf, taps=4, N=4096): 
    '''
    ...
    # shape (coarse chans, nfreq, taps*N)
...
def weight_upchan(cfu, U, taps=4):
    ...
    ''' 
    # creating an array containing the indices over which we need to sum the DFT over, shape = (1, taps*U)
    ...
    # result has shape (coarse chans, nfreq, taps*U)
...
def response_mtx(c, f, U, taps=4, N=4096):
    '''
...
def get_response_matrix(freqs, U, min_freq, max_freq, 
                        taps=4, fmin=300, number_channels=2048, sampling_rate=0.417):
    '''
    ...
    - number_channels: <int>
      number of channels. default is CHORD's 2048 channels
    - fmin: <float>
      minimum instrumental frequency. default is CHORD's 300 MHz
    - number_channels: <int>
      number of channels. default is CHORD's 2048
    ...
    f = np.reshape(freq_unit_strip(freqs[::-1],
                                   fmin=fmin,
                                   number_channels=number_channels,
                                   sampling_rate=sampling_rate), (freqs.size, 1))
...
def get_resampled_profiles(V, S, z, fine_freqs, b_max=77):
    ...
    for i in range(len(V)):
        new_prof = np.interp(fine_freqs, freqs[i][::-1], temps[i][::-1])
...
def channelize_catalogue(U, fstate, nside, catalogue_path, R_path, norm_path, freq_path, fine_freqs, output_path,  b_max=77):
    ...
    print('Up-channelizing spectra')
    responses, frequencies = upchannelize(profiles, U, R_path, norm_path, freq_path)
    pol = 'full'
